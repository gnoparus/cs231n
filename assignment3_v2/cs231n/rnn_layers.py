...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    a_t = x_prev_h.dot(Wxh) + b
    
    next_h = np.tanh(a_t)    
    cache = Wh, Wx, prev_h, x, a_t
...
#     dnext_h = np.sum(dh, axis=1)
    dnext_h = 0
        
    for t in range(T-1, -1, -1):   
#         print(t)
        dx_t, dprev_h_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:, t, :] + dnext_h, cache[t])
        dx[:, t, :] += dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
#         print(t)
        ...
        dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:, t, :] + dnext_h, dnext_c, cache[t])
        dx[:, t, :] += dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
